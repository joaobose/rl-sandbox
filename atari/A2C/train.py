...
def initialize(args):
    ...
    actor = Actor(actor_learning_rate, actor_lr_decay, 4, 4).to(device).float()
...
def preprocess_state(state):
    ...
    state = 0.07 * state[2, :, :] + 0.72 * \
        state[1, :, :] + 0.21 * state[0, :, :]  # grey value
    ...
    to_concat = (np.array([saved_state1]), np.array(
        [saved_state2]), np.array([saved_state3]), np.array([saved_state4]))
    full_state = np.concatenate(to_concat, axis=0)
...
def train(args):
    ...
    for episode in range(starting_episode, num_episodes):
        ...
        if autoshoot_first:
            time.sleep(0.05)
            state, _r, _, _, _ = env.step(1)  # shoot the ball
        ...
        while (not done):
            ...
            next_state, reward, terminated, truncated, game_data = env.step(
                action)
            done = terminated or truncated

            if game_data['lives'] < 5 and eps_end_on_life:
                done = True
