...
class QValues():
    ...
    def get_next(policy_net, target_net, next_state_batch, next_mask, device):
        ...
        if (rl == 'DQN'):
            next_state_values[next_mask] = target_net(
                next_state_batch.float()).max(1)[0].detach()
        elif (rl == 'DDQN'):
            # Get the best action for the next state using the policy network
            argmax = policy_net(next_state_batch.float()).max(1)[
                1].unsqueeze(0).t()
            # Get the value of the action selected by the policy, using the target
            next_state_values[next_mask] = target_net(
                next_state_batch.float()).gather(1, argmax).detach().t().squeeze(0)
