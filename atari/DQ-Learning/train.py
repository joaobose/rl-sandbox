...
def initialize(args):
    ...
    env = gym.envs.make(
        environ_atari, render_mode='human' if render_env else None)

    if CNN_arch == 'RESNET20':
        # Define policy and target networks
        policy_net = resnet20(batch_size, learning_rate,
                              env).to(device).float()
        target_net = resnet20(batch_size, learning_rate,
                              env).to(device).float()
    elif CNN_arch == 'DQN':
        policy_net = DQN(batch_size, learning_rate,
                         env.action_space.n).to(device).float()
        target_net = DQN(batch_size, learning_rate,
                         env.action_space.n).to(device).float()
...
def train(args):
    ...
    for episode in range(starting_episode, num_episodes):
        ...
        while (not done):
            ...
            if len(frames) == 3:
                agent.memory.push(Experience(concat_frames(
                    frames), action, reward, concat_next_frames(next_state, frames)))
        ...
        print('Episode: {} | Time: {}'.format(
            episode, round(time.time() - start, 2)))
        print('Steps done: {}'.format(agent.steps_done))
        print('Eps threshold: {}'.format(
            agent.strategy.get_exploration_rate(agent.steps_done)))
        print('learning_rate: {}'.format(
            agent.policy_net.optimizer.param_groups[0]['lr']))
